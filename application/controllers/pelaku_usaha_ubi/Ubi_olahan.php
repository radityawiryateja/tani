...
class Ubi_olahan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            'view' => 'pages/pelaku_usaha_ubi/ubi_olahan/index',
    ...
    public function load_table($row_no = 0)
	{
		...
		if (empty($related_kecamatan) && empty($related_desa)) {
			...
			if (!empty($related_kecamatan)) {
				$this->db->where_in('pelaku_ubi_olahan.id_kecamatan', $related_kecamatan);
			}

			if (!empty($related_desa)) {
				$this->db->where_in('pelaku_ubi_olahan.id_desa', $related_desa);
			}
		}

		$config = [
			'table' => 'pelaku_ubi_olahan',
			'select' => 'pelaku_ubi_olahan.*, kecamatan.nama AS kecamatan, desa.nama AS desa',
			'order_by' => ['pelaku_ubi_olahan.id' => 'asc'],
			'join' => [
				[
					'field' => 'kecamatan',
					'condition' => 'kecamatan.id = pelaku_ubi_olahan.id_kecamatan',
					'direction' => ''
				],
				[
					'field' => 'desa',
					'condition' => 'desa.id = pelaku_ubi_olahan.id_desa',
					'direction' => ''
				]
			],
			'group_by' => [],
			'where' => $where,
			'base_url' => base_url() . 'pelaku_usaha_ubi/ubi_olahan/index'
		];
    ...
    public function save()
	{
		...
		$nama_pelaku_usaha_ubi = $this->input->post('nama_pelaku_usaha_ubi', true);
		...
		if (!empty($_FILES['sk_pengukuhan']['name'])) {
            ...
            if ($id != '') {
                $old_file = $this->db->select('sk_pengukuhan')
                    ->get_where('pelaku_ubi_olahan', ['id' => $id])
                    ->row()->sk_pengukuhan;
		...
		if ($id == '') {
			...
			$save = $this->db->insert('pelaku_ubi_olahan', $data);
			$message = 'Kelompok Ubi Olahan berhasil disimpan';
		} else {
			$data['updated_by'] = $this->session->userdata('id');
			$data['updated_date'] = date('Y-m-d H:i:s');

			$save = $this->db->update('pelaku_ubi_olahan', $data, ['id' => $id]);
    ...
    public function edit()
    {
        ...
        $data = $this->db->get_where('pelaku_ubi_olahan', ['id' => $id])->row();
    ...
    public function delete()
    {
        ...
        if ($this->db->delete('pelaku_ubi_olahan', ['id' => $id])) {
            echo json_encode([
                'status' => true,
