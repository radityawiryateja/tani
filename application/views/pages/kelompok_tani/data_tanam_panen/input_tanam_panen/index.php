...
<div class="card shadow-sm">
    <div class="card-body">
		...
		<ul class="nav nav-tabs mb-3">
			<li class="nav-item">
				<a class="nav-link active" href="<?= base_url('kelompok_tani/input_tanam_panen') ?>">Input Tanam Panen</a>
			</li>
			<li class="nav-item">
				<a class="nav-link" href="<?= base_url('kelompok_tani/riwayat_tanam_panen') ?>">Riwayat Tanam Panen</a>
			</li>
		</ul>
            ...
            <table class="table table-striped text-nowrap align-middle mb-0">
                <thead>
                    <tr>
						...
						<th scope="col" style="min-width: 100px" class="align-middle">Estimasi Tanggal Panen</th>
...
<script>
    const config = {
        baseURL: '<?= base_url() . 'kelompok_tani/input_tanam_panen/' ?>',
        table: {
            name: 'tabelInputTanamPanen',
            columnTotal: 8,
        },
        formConfig: {
            afterSuccess: response => {
                dataTableInitializer.reload.tabelInputTanamPanen(0)
            }
        },
        sanitize: {
            numberOnly: true
    ...
    config.table.renderContent = (data, i) => {
        return data.map((val) => {
			...
			<?php if ($_SESSION['akses']->m_kel_tani == 1) : ?>
                panenButton +=
                    `<button onclick="panenData('${config.baseURL + 'edit'}', ${val.id})" type="" class="btn btn-primary">
                        <i class="ph ph-plant fs-5 align-middle"></i>
                    </button>
                `
            <?php endif; ?>
            ...
            return `
                <tr>
                    ...
                    <td class="text-center">${panenButton + editButton + deleteButton}</td>
					...
					<td>${val.estimasi_tgl_panen ? val.estimasi_tgl_panen : '-'}</td>
		...
		$.get(requestURL, {
			...
			openModal({
				...
				onUpdateOpen: () => {
					...
					$('#id_anggota_kel_tani').attr('disabled', true);
					$('#id_anggota_kel_tani_hidden').removeAttr('disabled');
