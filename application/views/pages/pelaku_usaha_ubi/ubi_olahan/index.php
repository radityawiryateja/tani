        ...
        <div class="modal-content">
            ...
            <form id="form">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
						<label for="nama_pelaku_usaha_ubi" class="form-label">Nama Kelompok Ubi Olahan <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="nama_pelaku_usaha_ubi" name="nama_pelaku_usaha_ubi" placeholder="Masukkan nama kelompok ubi olahan" autofocus>
                    </div>
        ...
        <div class="filters d-flex flex-md-row flex-column mb-4">
            <div class="d-flex flex-grow-1 justify-content-md-between me-md-3 mb-md-0 mb-3">
                ...
                <select class="form-select search-filter" name="search-by">
                    <option value="pelaku_ubi_olahan.nama">Nama Kelompok Ubi Olahan</option>
					<option value="pelaku_ubi_olahan.brand">Brand</option>
					<option value="kecamatan.nama">Nama Kecamatan</option>
					<option value="desa.nama">Nama Desa</option>
					<option value="pelaku_ubi_olahan.ketua">Nama Ketua</option>
					<option value="pelaku_ubi_olahan.sekretaris">Nama Sekretaris</option>
					<option value="pelaku_ubi_olahan.bendahara">Nama Bendahara</option>
                </select>
            </div>
...
<script>
    const config = {
        baseURL: '<?= base_url() . 'pelaku_usaha_ubi/ubi_olahan/' ?>',
        table: {
    ...
    config.table.renderContent = (data, i) => {
        return data.map((val) => {
			...
			<?php if ($_SESSION['akses']->m_pelaku_ubi_olahan == 1) : ?>
				memberButton += `
					<form action="<?= base_url('pelaku_usaha_ubi/data_anggota_kelompok_ubi_olahan') ?>" method="GET" style="display: inline;">
						<input type="hidden" name="id_pelaku_ubi_olahan" value="${val.id}">
		...
		$.get(requestURL, {
			...
			openModal({
				...
				onUpdateOpen: () => {
					...
					$('#nama_pelaku_usaha_ubi').val(response.nama);
