        ...
        <div class="modal-content">
            ...
            <form id="form">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
						<label for="nama_korporasi_ubi_jalar" class="form-label">Nama Kelompok Ubi Olahan <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="nama_korporasi_ubi_jalar" name="nama_korporasi_ubi_jalar" placeholder="Masukkan nama kelompok ubi olahan" autofocus>
                    </div>

					<div class="mb-3">
						<label for="brand" class="form-label">Brand <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="brand" name="brand" placeholder="Masukkan nama brand" autofocus>
                    </div>
					...
					<div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="ketua" name="ketua" placeholder="Masukkan nama ketua kelompok ubi olahan">
                    </div>

					<div class="mb-3">
                        <label for="sekretaris" class="form-label">Nama Sekretaris <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="sekretaris" name="sekretaris" placeholder="Masukkan nama sekretaris kelompok ubi olahan">
                    </div>

					<div class="mb-3">
                        <label for="bendahara" class="form-label">Nama Bendahara <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="bendahara" name="bendahara" placeholder="Masukkan nama bendahara kelompok ubi olahan">
                    </div>
...
<header class="mb-3">
    <div class="row">
        <div class="col-md-8">
            <h1 class="fw-bold">Daftar Kelompok Ubi Olahan</h1>
            <p>Pengelolaan data Kelompok Ubi Olahan</p>

			<!-- breadcrum -->
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb p-0">
					<li class="breadcrumb-item"><a href="<?= base_url() ?>">Beranda</a></li>
					<li class="breadcrumb-item active" aria-current="page">Kelompok Ubi Olahan</li>
				</ol>
			</nav>
        </div>
        <?php if ($_SESSION['akses']->m_korporasi_ubi_olahan == 1) : ?>
            <div class="col-md-4 text-md-end">
                <button onclick="createNewData()" class="btn btn-primary"><i class="ph ph-plus-circle fs-4 align-middle me-1"></i>
                    <span class="align-middle">Tambah Kelompok Ubi Olahan</span>
				</button>
            </div>
        <?php endif; ?>
    </div>
        ...
        <div class="filters d-flex flex-md-row flex-column mb-4">
            <div class="d-flex flex-grow-1 justify-content-md-between me-md-3 mb-md-0 mb-3">
                ...
                <select class="form-select search-filter" name="search-by">
                    <option value="korporasi_ubi_olahan.nama">Nama Kelompok Ubi Olahan</option>
					<option value="korporasi_ubi_olahan.brand">Brand</option>
					<option value="kecamatan.nama">Nama Kecamatan</option>
					<option value="desa.nama">Nama Desa</option>
					<option value="korporasi_ubi_olahan.ketua">Nama Ketua</option>
					<option value="korporasi_ubi_olahan.sekretaris">Nama Sekretaris</option>
					<option value="korporasi_ubi_olahan.bendahara">Nama Bendahara</option>
                </select>
            </div>
            ...
            <table class="table table-striped text-nowrap align-middle mb-0">
                <thead>
                    <tr>
						...
						<th scope="col" rowspan="2" style="min-width: 100px" class="align-middle">Brand</th>
...
<script>
    const config = {
        baseURL: '<?= base_url() . 'korporasi_ubi_jalar/ubi_olahan/' ?>',
        table: {
            name: 'tabelPelakuUbiOlahan',
            columnTotal: 12,
        },
        formConfig: {
            afterSuccess: response => {
                dataTableInitializer.reload.tabelPelakuUbiOlahan(0)
            }
        },
        sanitize: {
            numberOnly: true
    ...
    config.table.renderContent = (data, i) => {
        return data.map((val) => {
			...
			<?php if ($_SESSION['akses']->m_korporasi_ubi_olahan == 1) : ?>
				memberButton += `
					<form action="<?= base_url('korporasi_ubi_jalar/data_korporasi_anggota_kelompok_ubi_olahan') ?>" method="GET" style="display: inline;">
						<input type="hidden" name="id_korporasi_ubi_olahan" value="${val.id}">
...
<script>
    const editData = (requestURL, id) => {
		...
		$.get(requestURL, {
			...
			openModal({
				state: 'update',
				modalContext: 'Kelompok Ubi Olahan',
				onUpdateOpen: () => {

					$('input[name="id"]').val(response.id);
					$('#id_kecamatan').val(response.id_kecamatan);
					$('#id_desa').val(response.id_desa);
					$('#nama_korporasi_ubi_jalar').val(response.nama);
